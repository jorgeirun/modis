/******************************************************************************
    ...
    PURPOSE:  The purpose of this class is to create a GUI interface for
              application and allow clients to edit the projection parameters.
    ...
    a compiler requirement regarding JDK package (version 2.0+).

    PROJECT:  MRTSwath
    NOTES:
    1. The Graphical User Interface (GUI) design) in this application is based
       on Java swing family class. It extends/derives from JDialog by
...
class ProjParamDialog extends JDialog
{
    ...
    private String [] ellipsoidstrings = {
       "Airy", "Australian National", "Bessel", "Clarke 1866", "Clarke 1880",
       "Everest", "GRS 1980", "Hough", "International 1909",
       "International 1967", "Krassovsky", "Mercury 1960", "Modified Airy",
       "Modified Everest", "Modified Mercury 1968", "MODIS Sphere",
       "Southeast Asia", "Sphere 19", "Walbeck", "WGS 1966", "WGS 1972",
       "WGS 1984", "No Ellipsoid"};
    private String[][] projInfo =
    {
        ...
        { "Sphere", "", "", "", "", "", "", "", "", "", "", "", "", "", ""},
            /* IGH */
        { "Sphere", "", "", "", "CentMer", "", "FE", "FN", "NZone", "", 
          "RFlag", "","", "", ""},  /* ISIN */
        { "Sphere", "", "", "", "CentLon", "CenterLat", "FE", "FN", "",
          "", "", "", "", "", ""},  /* Lambert Azimuthal */
        { "SMajor", "SMinor", "STDPR1", "STDPR2", "CentMer", "OriginLat",
          "FE", "FN", "", "", "", "", "", "", ""},  /* LCC */
        { "Sphere", "", "", "", "CentMer", "", "FE", "FN", "", "", "",
          "",  "",  "", ""},  /* Mollweide */
        ...
        { "Lon/Z", "Lat/Z", "", "", "", "", "", "", "", "", "", "", "", "",
          ""},  /* UTM */
        { "SMajor", "SMinor", "Factor", "", "CentMer", "OrginLat", "FE", "FN",
          "","","","","","",""},  /* TM */
        { "SMajor", "SMinor", "STDPR1", "STDPR2", "CentMer", "OriginLat",
          "FE", "FN", "",  "",  "",  "",  "",  "",  ""},  /* Albers EA */
    ...
    javax.swing.JComboBox ellipsoidcombo =
        new javax.swing.JComboBox(ellipsoidstrings);           
...
/******************************************************************************
    ...
    PURPOSE:  To construct an editing dialog box for the user to edit
...
/******************************************************************************
    NAME:     initComponents()
    PURPOSE:  To initialze an editing dialog box. This function is called after
...
******************************************************************************/
    public void initComponents() throws Exception
    {
        ...
        utmText.setLocation(new java.awt.Point(90, 300));

        utmLabel.setSize(new java.awt.Dimension(70, 20));
        utmLabel.setVisible(true);
        utmLabel.setLocation(new java.awt.Point(20, 300));
                
        combolabel.setSize(new java.awt.Dimension(60,25));
        combolabel.setVisible(true);
        combolabel.setLocation(new java.awt.Point(20,270));
        ...
        setSize(new java.awt.Dimension(577, 360));
        getContentPane().setLayout(null);
        setTitle("mrtswath.ProjParamFrame");
        ...
        ellipsoidcombo.setSize(new java.awt.Dimension(180,25));
        ellipsoidcombo.setVisible(true);
        ellipsoidcombo.setLocation(new java.awt.Point(90,270));
        getContentPane().add(ellipsoidcombo);
        ellipsoidcombo.setEnabled(true);

        getContentPane().add(combolabel);
        combolabel.setText("Ellipsoid:");
        ...
        ellipsoidcombo.addActionListener(new java.awt.event.ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent e)
            {
                ellipsoidcomboActionPerformed(e);
            }
        });
...
/******************************************************************************
    NAME:     addNotify()
    PURPOSE:  To add action listner for the appropriate response.
...
/******************************************************************************
    ...
    PURPOSE:  To close the window (set inviible) when the close box is clicked
...
/******************************************************************************
    ...
    PURPOSE:  To respond to cancel button of this dialog
...
/******************************************************************************
    ...
    PURPOSE:  To clear and disable all editing box
...
/******************************************************************************
    NAME:     getResult()
    PURPOSE:  To retrieve the result.
    ...
    PROJECT:  MRTSwath
    NOTES:
    1. Final means that this function cannot be derived.
******************************************************************************/
    final int getResult() { return result; }

/******************************************************************************
    NAME:     saveValuesToModel()
    PURPOSE:  To save all values from editing box to model.
    ...
    public void saveValuesToModel() throws NumberFormatException
    {
        ...
        for (int i=0; i<15; i++)
        {
            if (projInfo[projectionId][i].length() > 0)
            {
                try
                {
                    projValue[i] = Double.valueOf(text[i].getText()).
                        doubleValue();
                }
                catch (NumberFormatException e)
                {
                    new JOptionPane("Invalid value for parameter: " +
                        projInfo[projectionId][i]).createDialog(this,
                        "Bad Parameter Value").setVisible(true);
                    throw e;
                }
            }
            else
        ...
        if (text[15].getText().length() > 0)
        {
            ...
            if (utm < -60 || utm > 60)
            {
                complain ("Wrong Value",
                    "UTM zone must be an integer value from -60 to 60. " +
                    "Please try again...");
                return;
            }
            else
        ...
        model1.setEllipsoid(s);
    }

/*****************************************************************************
    NAME:           ellipsoidcomboActionPerformed()
    ...
    NOTES:
*****************************************************************************/
    public void ellipsoidcomboActionPerformed(java.awt.event.ActionEvent event)
    {
...
/******************************************************************************
    NAME:    setupFields()
    PURPOSE: To set up various fields
...
******************************************************************************/
    public void setupFields(int projId)
    {
        ...
        ellipsoidcombo.setSelectedIndex(model.getEllipsoidInt());
    }               
